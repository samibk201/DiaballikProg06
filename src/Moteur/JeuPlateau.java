...
public class JeuPlateau {
	...
	public void init(int l, int h, int disposition) {
		...
		case 0:// aucun adverse pion dans un camps adverse
			...
			for (int j = 0; j < h; j++)
				if (j == h / 2)
					cases[0][j] = new Case(joueurs[0], OC_BALLE);
				else
					cases[0][j] = new Case(joueurs[0], OCCUPE);
			// joueur 2
			for (int j = 0; j < h; j++) {
				if (j == h / 2)
					cases[l - 1][j] = new Case(joueurs[1], OC_BALLE);
				else
					cases[l - 1][j] = new Case(joueurs[1], OCCUPE);
			}
			break;
	...
	/**
	 * TO DO: Il Faut gérer le lancement de la balle Mdofier la fonction pour
	 * qu'elle renvoie un boolean en fonction que le coup soit valide ou pas
	 **/
	...
	boolean testENJr(int i, int j, Joueur joueur) {
		if (cases[i][j].getEtat() == LIBRE || (cases[i][j].getEtat() == OCCUPE && joueur == cases[i][j].getJoueur()))
			return true;
		return false;
	}
	...
	boolean verifieDest(int x, int y, int destX, int destY, Joueur joueur) {
		if ((joueur.getNum() != getCase(destX, destY).getJoueur().getNum()))
			...
			if (x < destX) {
				int i = x + 1, j = y;
				while (i < destX && (testENJr(i, j, joueur)))
					i++;
				if (i != destX)
					return false;
			}
			...
			if (destY < y) {
				int i = y - 1, j = x;
				while (i > destY && (testENJr(i, j, joueur)))
					i--;
				if (i != destY)
					return false;
			}
			...
			if (destX < x && destY < y) {// cas montée vers la gauche
				int i = x - 1, j = y - 1;
				while ((i > destX && j > destY) && (testENJr(i, j, joueur))) {
					i--;
					j--;
				}
				if (i != destX || j != destY)
	...
	/**
	 * TO DO:Cette fonction doit être modifier pour renvoyer un boolean en fonction
	 * que ça soit permis le déplacement ou pas
	 **/
	...
	boolean deplacerVers(int action, Joueur joueur, int x, int y) {
		...
		case 0:// Avancer
			...
			if (joueur.getNum() == 1) {
				...
				if (0 <= (x - 1) && (x - 1) < hauteur && getCase(x - 1, y).getEtat() == LIBRE) {
					modifeCase(x, y, x - 1, y, joueur);
				} else {
					System.out.println("Impossible d'avancer");
					return false;
	...
	boolean joueurXGagne() {
		int j = 0;
		while (j < this.hauteur && cases[0][j].getEtat() != OC_BALLE)
			j++;
		if (j < this.hauteur && cases[0][j].getJoueur().getNum() == 2) {
			System.out.println("Bravo " + cases[0][j].getJoueur().getNom() + ":" + cases[0][j].getJoueur().getNum()
					+ " vous avez gagné ");
			return true;
		}
		j = 0;
		while (j < this.hauteur && cases[this.largeur - 1][j].getEtat() != OC_BALLE)
			j++;
